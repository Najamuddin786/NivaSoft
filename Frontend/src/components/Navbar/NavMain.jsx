...
function Menu(){
    ...
    return <Box >
                ...
                <HStack display={{base:"none",md:'flex'}} px='2' fontWeight={{base:"500",lg:"600"}}>
                    {navMenu.map((e,i)=>{
                        return <Link to={e.link} key={e.id}><Text className="nav-item" color={e.name=="Contact Us" ? "white" :''} borderRadius={'sm'} p='1' bg={e.name=='Contact Us' ? "rgb(2,0,36)" :""}>{e.name}</Text></Link>
                    })}
                </HStack>
                ...
                {hide ? <HStack  gap='5' className="animated-box" bg='blue.400' color={'white'} boxShadow={'xl'} borderRadius={'sm'} py='10'  flexDirection={'column'} mx={{base:"2vw"}} top='85px' position={'fixed'} left={'0px'} w={'96vw'}  display={{base:"flex",md:'none'}} px='2' fontWeight={{base:"500",lg:"600"}}>
                    {navMenu.map((e,i)=>{
                        return <Link to={e.link} key={e.id}><Text onClick={()=>setHide(false)} className="nav-item" color={e.name=="Contact Us" ? "white" :''} borderRadius={'sm'} px='2' py='1' bg={e.name=='Contact Us' ? "rgb(26, 24, 68)" :""}>{e.name}</Text></Link>
                    })}
...
export default function NavMain(){
    ...
    return <>
            <HStack zIndex={'1500'} bg='white'  position={'sticky'} top='15px' justifyContent={'space-between'} className="animated-box"  borderRadius={'md'} mt='15px'  mx={{base:"2vw",md:"5vw",lg:"10vw"}} style={{boxShadow:"1px 0px 10px rgba(0, 0, 255, 0.388)"}}>
